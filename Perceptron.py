    ...
    def train_data_set(self, X_train, Y_train):
        ...
        for i in range(self.max_iteration):
            ...
            for i in range( X_train.shape[0] ):
                ...
                if value >= 0:
                    if Y_train[i] == 0:
                        missmatch_list.append(i)
                        jog_biyog_select.append(-1)
            ...
            for i in range ( len( missmatch_list ) ):
                fixed_X = X_train[ missmatch_list[i] ].tolist()
                fixed_X.append(1)
                ultra_X = np.array( fixed_X )

                new_w = new_w + ( jog_biyog_select[i] * ultra_X )
